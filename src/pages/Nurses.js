...
export default function NursesPage() {
  ...
  const t = new Tabs('nurse', tabIndex, tabs, setTabs, setSelectedTab, setTabIndex, newBtn, lg);
        ...
        <Main open={open}>
          <Grid container mb={'0px'} mt={6} >
            ...
            <Grid item xs={12} md={6} xl={6} textAlign={'end'}  >
              <Button id="newButton" variant="outlined" onClick={t.onOpenTabClick} ref={newBtn} justifyContent="flex-end">
                <PeopleIcon />{lg.get('Add nurse')}</Button>
            </Grid>
          </Grid>
          <Box sx={{ width: '100%' }}>
            <TabContext value={selectedTab ? selectedTab : '1'}>
              ...
              <TabPanel value="Main" style={{ padding: "1px" }}>
                <Nurses openUser={t.openTab} openPatients={t.openTab} />
              </TabPanel>
              {tabs.map(panel => (
